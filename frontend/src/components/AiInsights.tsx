    ...
    <Card className="border border-[#DBEAFE] bg-[#DBEAFE]/30 shadow-sm overflow-hidden relative rounded-3xl backdrop-blur-sm">
      ...
      <CardHeader className="px-8 pt-8 pb-4 relative z-10">
        <div className="flex items-center justify-between gap-4">
          <CardTitle className="flex items-center gap-3 text-[#000000] font-headline font-extrabold tracking-tight text-xl">
          ...
          {!insights && !loading && (
            <Button size="lg" onClick={fetchInsights} className="bg-[#4F8DF7] hover:bg-[#2563EB] text-white font-semibold text-[11px] uppercase tracking-[0.12em] px-6 h-12 rounded-xl shadow-xl shadow-[#4F8DF7]/20 transition-all active:scale-95">
              Execute Analysis
            </Button>
          )}
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16 gap-6 bg-white/40 rounded-2xl border border-white/60 shadow-inner">
            ...
            <div className="text-center">
              <p className="text-sm font-semibold text-[#000000] uppercase tracking-[0.12em] mb-1">Synthesizing Data</p>
              <p className="text-[11px] font-medium text-gray-500 uppercase tracking-[0.08em]">Aggregating market trends & indicators...</p>
            </div>
          </div>
        ) : insights ? (
          <div className="space-y-8">
            <div className="flex items-center gap-4 bg-white/50 p-4 rounded-2xl border border-white/80">
              <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-[0.12em]">Market Signal</span>
              <Badge variant="secondary" className={cn(
                "capitalize px-4 py-1.5 rounded-lg font-semibold text-[11px] uppercase tracking-[0.12em] border",
            ...
            <div className="space-y-4 bg-white/40 p-6 rounded-2xl border border-white/60 shadow-inner">
              <h4 className="text-[10px] font-semibold text-gray-500 uppercase tracking-[0.12em]">Strategic Abstract</h4>
              <p className="text-lg text-[#1F2937] leading-relaxed font-semibold tracking-tight italic opacity-90">
                "{insights.summary}"
              </p>
            </div>

            <div className="space-y-4">
              <h4 className="text-[10px] font-semibold text-gray-500 uppercase tracking-[0.12em] px-1">Core Indicators</h4>
              <ul className="space-y-3">
                {insights.keyInsights.map((insight, idx) => (
                  <li key={idx} className="flex gap-4 p-4 bg-white/60 rounded-2xl border border-white/80 group hover:bg-white transition-colors duration-300">
                    ...
                    <span className="text-sm font-medium text-[#1F2937] leading-snug">{insight}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="text-center py-10 bg-white/40 rounded-2xl border border-white/60 shadow-inner">
            <p className="text-sm font-medium text-[#1F2937] opacity-65 tracking-tight px-6 leading-relaxed">
