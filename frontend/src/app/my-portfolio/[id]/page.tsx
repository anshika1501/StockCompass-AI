...
import {
  ...
  Target, Leaf, Calendar, Layers, Loader2, AlertCircle,
  Trash2, ArrowUpDown, ChevronUp, ChevronDown, LogIn, RefreshCw,
} from "lucide-react";
import {
...
import {
  getPortfolioDetail, deleteHolding, getToken,
  type PortfolioDetail, type EnrichedHolding,
...
const PIE_COLORS = [
  "#4F8DF7", "#6C63FF", "#34D399", "#F59E0B",
  "#F87171", "#60A5FA", "#A78BFA", "#FCD34D",
];
...
function SummaryCard({
  icon, label, value, sub, positive, neutral,
}: {
  ...
  return (
    <div className="flex flex-col justify-between rounded-2xl border border-slate-800/60 bg-slate-900 p-5 shadow-md">
      <div className="mb-3 flex items-center gap-2">
        <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-800 text-[#4F8DF7]">
          {icon}
        </span>
...
export default function PortfolioDetailPage() {
  ...
  const weightData = useMemo(() =>
    (data?.holdings ?? []).map((h) => ({
      name: h.ticker.replace(".NS", ""),
      value: data
        ? Math.round((h.current_value / data.summary.current_value) * 1000) / 10
        : 0,
    })), [data]);
  ...
  if (loading) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4">
        ...
        <p className="text-sm text-slate-500">Fetching live prices for your holdings…</p>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4">
        ...
        <p className="text-sm text-slate-700">{error ?? "Portfolio not found."}</p>
        <button onClick={() => router.back()} className="text-sm font-semibold text-[#4F8DF7] hover:underline">
          ← Go back
        </button>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19] text-white">
      <div className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        ...
        {/* ── Holdings Table ── */}
        <section className="overflow-hidden rounded-2xl border border-slate-800/60 bg-slate-900 shadow-xl">
          {/* toolbar */}
          <div className="flex flex-col gap-3 border-b border-slate-800 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-2 text-sm text-white placeholder:text-slate-500 focus:border-[#4F8DF7] focus:outline-none sm:w-64"
            />
          </div>
              ...
              <thead>
                <tr className="border-b border-slate-800 text-[10px] font-semibold uppercase tracking-wider text-slate-500">
                  {([
                    ...
                    <th
                      key={label}
                      className={`px-4 py-3 ${key ? "cursor-pointer select-none hover:text-white" : ""}`}
              ...
              <tbody className="divide-y divide-slate-800/60">
                {filteredHoldings.length === 0 ? (
                  ...
                  filteredHoldings.map((h) => {
                    const gain = h.pnl >= 0;
                    return (
                      <tr key={h.id} className="transition hover:bg-slate-800/40">
                        <td className="px-4 py-3">
                          <span className="font-bold text-white">{h.ticker}</span>
                        </td>
                        <td className="max-w-[160px] truncate px-4 py-3 text-slate-400">{h.company_name}</td>
                        <td className="px-4 py-3 tabular-nums text-slate-300">{h.quantity}</td>
                        <td className="px-4 py-3 tabular-nums text-slate-300">{INR(h.buy_price)}</td>
                        <td className="px-4 py-3 tabular-nums font-medium text-white">{INR(h.current_price)}</td>
                        <td className="px-4 py-3 tabular-nums text-slate-300">{INR(h.invested)}</td>
                        <td className="px-4 py-3 tabular-nums font-medium text-white">{INR(h.current_value)}</td>
                        <td className={`px-4 py-3 tabular-nums font-semibold ${gain ? "text-emerald-400" : "text-rose-400"}`}>
                          {(gain ? "+" : "-") + INR(h.pnl)}
                        </td>
                        <td className={`px-4 py-3 tabular-nums font-semibold ${gain ? "text-emerald-400" : "text-rose-400"}`}>
                          {pct(h.pnl_pct)}
                        ...
                        <td className="px-4 py-3">
                          <span className={`rounded-full border px-2.5 py-0.5 text-[10px] font-bold ${RISK_COLORS[h.risk_tag] ?? RISK_COLORS.Unknown}`}>
                            {h.risk_tag}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <button
                            ...
                            className="inline-flex items-center gap-1 rounded-lg border border-rose-800/60 bg-rose-950/40 px-3 py-1.5 text-xs font-semibold text-rose-400 transition hover:bg-rose-600 hover:text-white disabled:opacity-40"
                          >
                            {deletingId === h.id ? (
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-800/60 bg-slate-900 shadow-xl">
          {/* Tab bar */}
          <div className="flex gap-1 border-b border-slate-800 px-5 py-3">
          ...
          <div className="p-6">
            {tab === "performance" && (
              <div>
                <p className="mb-4 text-sm font-semibold text-slate-400">Invested vs Current Value per Stock</p>
                {performanceData.length === 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height={340}>
                    <BarChart data={performanceData} barCategoryGap="30%">
                      ...
                      <Bar dataKey="Invested" fill="#334155" radius={[6, 6, 0, 0]} />
              ...
              <div className="flex flex-col items-center gap-6 lg:flex-row">
                ...
                <ul className="min-w-[200px] space-y-2">
                  {(tab === "weight" ? weightData : allocationData).map((d, i) => (
                    <li key={`${d.name}-${i}`} className="flex items-center gap-2 text-xs text-slate-300">
            ...
            {tab === "dividends" && (
              <div>
                <p className="mb-4 text-sm font-semibold text-slate-400">Estimated Annual Dividends per Stock</p>
